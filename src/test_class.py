...
class TestClass:
    ...
    def test_norm_data(self):
        assert len((self.pd.data.norm_data_ls[0]['High'].values>1.01).nonzero()[0]) ==0
        assert len((self.pd.data.norm_data_ls[0]['Low'].values>1.01).nonzero()[0])==0
        assert len((self.pd.data.norm_data_ls[0]['Open'].values>1.01).nonzero()[0])==0
        assert len((self.pd.data.norm_data_ls[0]['Close'].values>1.01).nonzero()[0])==0
